...
namespace FineUI
{
    ...
    public class Tree : CollapsablePanel, IPostBackDataHandler, IPostBackEventHandler
    {
        ...
        protected override void OnFirstPreRender()
        {
            ...
            postBackScript = postBackScript.Replace("#ID#'", "'+op.node.id");
            listenersBuilder.AddProperty("beforeload", String.Format("function(store,op){{if(op.action==='read'&&op.node.id!=='root'){{{0}}}return false;}}", postBackScript), true);
            ...
            string itemclickScript = "var args='Command$'+node.getId()+'$'+node.data.f_commandname+'$'+node.data.f_commandargument;";
            itemclickScript += GetPostBackEventReference("#Click#").Replace("'#Click#'", "args");
            itemclickScript = String.Format("if(this.rendered&&node.data.f_enableclickevent){{{0}}}", itemclickScript);
            itemclickScript = "if(this.rendered&&node.data.f_clientclick){if(new Function(node.data.f_clientclick)()===false){return;}}" + itemclickScript;
            //itemclickScript = "if(node.data.f_clientclick){eval(node.data.f_clientclick);}" + itemclickScript; // new Function(node.data.f_clientclick)();
            ...
            checkchangeScript = String.Format("if(this.rendered&&node.data.f_enablecheckevent){{{0}}}", checkchangeScript);
            ...
            expandScript = String.Format("if(this.rendered&&node.data.f_enableexpandevent){{{0}}}", expandScript);
            ...
            collapseScript = String.Format("if(this.rendered&&node.data.f_enablecollapseevent){{{0}}}", collapseScript);
