...
namespace FineUI
{
    ...
    public class PageManager : ControlBase, IPostBackEventHandler
    {
        ...
        public string LoadingImageUrl
        {
            get
            {
                object obj = FState["LoadingImageUrl"];
                return obj == null ? "" : (string)obj;
            }
            set
            {
                FState["LoadingImageUrl"] = value;
            }
        }
        ...
        [Description("自定义页面加载图片")]
        [Editor(typeof(ImageUrlEditor), typeof(UITypeEditor))]
        [Obsolete("已废除，请使用LoadingImageUrl")]
        ...
        /// </summary>
        [Category(CategoryName.OPTIONS)]
        [DefaultValue(ConfigPropertyValue.LOADING_IMAGE_NUMBER_DEFAULT)]
        [Description("加载图片的数字编码")]
        public int LoadingImageNumber
        {
            get
            {
                object obj = FState["LoadingImageNumber"];
                if (obj == null)
                {
                    if (DesignMode)
                    {
                        return ConfigPropertyValue.LOADING_IMAGE_NUMBER_DEFAULT;
                    }
                    else
                    {
                        return GlobalConfig.GetLoadingImageNumber();
                    }
                }
                return (int)obj;
            }
            set
            {
                FState["LoadingImageNumber"] = value;
            }
        }
        ...
        public bool EnableFStateValidation
        {
            get
            {
                object obj = FState["EnableFStateValidation"];
                if (obj == null)
                {
                    if (DesignMode)
                    {
                    ...
                    {
                        return GlobalConfig.GetEnableFStateValidation();
                    }
                }
                return (bool)obj;
            }
            set
            {
                FState["EnableFStateValidation"] = value;
            }
        }
        ...
        internal static readonly string[] LOADING_IMAGES_MAP = new string[] { 
            "3", "1", "2", "67", "6", "53", 
            "74", "13", "21", "25", "61", "62", 
            "109", "72", "75", "76", "92", "93", 
            "104", "8", "31", "43", "54", "68", 
        ...
        /// 获取加载图片路径
        /// </summary>
        /// <param name="loadingImageNumber"></param>
        /// <returns></returns>
        public string GetLoadingImageUrl(int loadingImageNumber)
        {
            // 映射到文件夹名称
            if (loadingImageNumber > LOADING_IMAGES_MAP.Length)
            {
                loadingImageNumber = LOADING_IMAGES_MAP.Length;
            }
            string loadingImageFolder = "_" + LOADING_IMAGES_MAP[loadingImageNumber - 1];

            return ResolveUrl("~/res/loading/") + loadingImageFolder + "/" + "ffffff_444444.gif";
        }
        ...
        public string GetLoadingImageUrl()
        {
            ...
            {
                imageUrl = ResolveUrl(LoadingImageUrl);
            }
            else
            {
                imageUrl = GetLoadingImageUrl(LoadingImageNumber);
            }
        ...
        protected override void RenderBeginTag(HtmlTextWriter writer)
        {
            if (EnablePageLoading)
            {
                ...
                string loadingImageUrl = GetLoadingImageUrl();

                //content = content.Replace("#LOADING_IMAGE_SRC#", imageUrl);

                StringBuilder sb = new StringBuilder();
                sb.Append("<div id=\"f_loading_mask\" class=\"f-loading-mask\"></div>");
                sb.Append("<div id=\"f_loading\" class=\"f-loading\"><div class=\"f-loading-img");
                if (!String.IsNullOrEmpty(LoadingImageUrl))
                {
                    sb.Append("f-loading-img-custom");
                }
                sb.AppendFormat("\"><img src=\"{0}\"/></div></div>", loadingImageUrl);
