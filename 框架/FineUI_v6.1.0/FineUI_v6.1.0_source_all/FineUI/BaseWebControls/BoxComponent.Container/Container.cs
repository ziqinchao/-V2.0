        ...
        protected override void OnFirstPreRender()
        {
            ...
            if (Layout != Layout.Container)
            {
                ...
                else if (Layout == Layout.HBox || Layout == Layout.VBox)
                {
                    ...
                    // extjs 6.0: Another important change to Component is the removal of the margins config, as well as the defaultMargins config of all container layouts. The margin config is now the only way to configure margins on Components. Applications that previously used margins should change to using margin instead. Applications that used defaultMargins on the layout should instead configure a margin in the Container’s defaults block.
                    if (BoxConfigChildMargin != "0")
                    {
                        ...
                        defaults.AddProperty("margin", BoxConfigChildMargin);
