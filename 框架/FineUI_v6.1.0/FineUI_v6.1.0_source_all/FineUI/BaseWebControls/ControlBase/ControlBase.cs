...
namespace FineUI
{
    ...
    public abstract class ControlBase : Control, INamingContainer
    {
        ...
        internal JObject PostBackState
        {
            get
            {
                if (_postBackState == null)
                {
                    ...
                    {
                        JArray postBackStateArray = states.Value<JArray>(ClientID);
                        if (postBackStateArray != null)
                        {
                            _postBackState = (JObject)postBackStateArray[0];

                            // 如果回发状态不会空，则需要验证
                            string stateValidationString = (string)postBackStateArray[1];

                            if (String.IsNullOrEmpty(stateValidationString) || !ValidateFState(_postBackState, stateValidationString))
                            {
                                throw new Exception(String.Format("无效的 F_STATE 状态信息（{0}）。", ID));
                            }
                        }
                    }
                    else
        ...
        protected virtual void OnAjaxPreRender()
        {
            ...
            {
                // 更新当前控件的 F_STATE 状态
                string fstate = ConvertPropertiesToJObject(currentModifiedProperties).ToString(Formatting.None);

                if (PageManager.Instance.EnableFStateValidation)
                {
                    // f_state_v是对整个f_state的验证，而非某个属性
                    List<string> totalModifiedProperties = FState.GetTotalModifiedProperties();
                    JObject fstateall = ConvertPropertiesToJObject(totalModifiedProperties);

                    sb.AppendFormat("F.f_state({0},{1},'{2}');", XID, fstate, GetFStateValidation(fstateall));
                }
                else
                {
                    sb.AppendFormat("F.f_state({0},{1});", XID, fstate);
                }
            }
        ...
        protected virtual void OnFirstPreRender()
        {
            ...
            {
                JObject fstateall = ConvertPropertiesToJObject(totalModifiedProperties);
                string fstate = fstateall.ToString(Formatting.None);

                // 注：作为变量提取出来，是因为JS端构造函数中可能会用到这个变量
                AddStartupScript(String.Format("var {0}={1};", GetFStateScriptID(), fstate));
        ...
        #region GetFStateValidation ValidateFState
        ...
        {
            string fstate = stateObj.ToString(Newtonsoft.Json.Formatting.None);
            string fstateCode = GetShortMD5HashWithSeed(fstate); //.GetHashCode().ToString();
